...
SPECIALS_V7 = ["www", "@", "?", "&", "#", "%", ".", "=", "_", "-"]  # ✅ http, https 제거됨
...
# ✅ 특징 추출 v7
def extract_lexical_features_v7(url):
    try:
        ...
        special_counts = [url.lower().count(s) for s in SPECIALS_V7]
...
# ✅ 피처 이름 v7
def get_feature_names_v7():
    return [
        "url_len", "hostname_len", "tld_len", "path_len",
        *[f"count_{s}" for s in SPECIALS_V7],
...
if __name__ == "__main__":
    ...
    features = [extract_lexical_features_v7(url) for url in tqdm(df["url"], desc="🔍 V7 특징 추출 중")]
    feature_df = pd.DataFrame(features, columns=get_feature_names_v7())
    df_with_features = pd.concat([df.reset_index(drop=True), feature_df], axis=1)
    df_with_features.to_csv("dataset/train_dataset_with_features_v7.csv", index=False)
    print("✅ 저장 완료: dataset/train_dataset_with_features_v7.csv")
