...
if __name__ == "__main__":
    ...
    features = [extract_lexical_features_v6(url) for url in tqdm(df["url"], desc="🔍 V6 특징 추출 중")]
    feature_df = pd.DataFrame(features, columns=get_feature_names_v6())
    df_with_features = pd.concat([df.reset_index(drop=True), feature_df], axis=1)
    # df_with_features.to_csv("dataset/final_url_dataset_with_features_v6.csv", index=False)
    # print("✅ 저장 완료: dataset/final_url_dataset_with_features_v6.csv")
    df_with_features.to_csv("dataset/train_dataset_with_features_v6.csv", index=False)
    print("✅ 저장 완료: dataset/train_dataset_with_features_v6.csv")
