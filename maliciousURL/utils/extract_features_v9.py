...
# ✅ v9 특징 추출
def extract_lexical_features_v9(url):
    try:
        ...
        special_counts = [url.lower().count(s) for s in SPECIALS_V9]
        ...
        subdomain = prime_url.split('.')[0]
        has_hyphen_in_subdomain = int('-' in subdomain)
        hyphen_and_long_path = int(has_hyphen_in_subdomain and path_len >= 50)
        ...
        return [
            ...
            *special_counts, digit_count,
            ...
            other_domain_len, suspicious_http, has_at_symbol,
            has_hyphen_in_subdomain, hyphen_and_long_path, typo_score
        ]
    except:
        return [0] * 32

def get_feature_names_v9():
    return [
        "url_len", "hostname_len", "tld_len", "path_len",
        *[f"count_{s}" for s in SPECIALS_V9],
        ...
        "has_base64", "query_len", "fragment_len",
        "other_domain_len", "suspicious_http", "has_at_symbol",
        "has_hyphen_in_subdomain", "hyphen_and_long_path", "typosquatting_score"
    ]
...
if __name__ == "__main__":
    ...
    features = [extract_lexical_features_v9(url) for url in tqdm(df["url"], desc="🔍 V9 특징 추출 중")]
    feature_df = pd.DataFrame(features, columns=get_feature_names_v9())
    df_with_features = pd.concat([df.reset_index(drop=True), feature_df], axis=1)
    df_with_features.to_csv("dataset/train_dataset_with_features_v9.csv", index=False)
    print("✅ 저장 완료: dataset/train_dataset_with_features_v9.csv")
