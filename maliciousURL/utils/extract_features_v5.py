...
# ✅ 버전 5 특징 추출 함수
def extract_lexical_features_v5(url):
    try:
        ...
        # ✅ other_domain 길이 추가 (path 기준 첫 토큰)
        other_domain = path.lstrip('/').split('/')[0] if path else ''
        other_domain_len = len(other_domain)
        ...
        return [
            ...
            *special_counts, digit_count,
            ...
            other_domain_len  # ✅ 추가된 26번째 feature
        ]
    except:
        return [0] * 26

# ✅ 특성 이름 정의
def get_feature_names_v5():
    return [
        "url_len", "hostname_len", "tld_len", "path_len",
        *[f"count_{s}" for s in SPECIALS],
        ...
        "special_ratio", "path_is_numeric", "has_query_string",
        "has_base64", "query_len", "fragment_len",
        "other_domain_len"  # ✅ 마지막 feature 이름
    ]

if __name__ == "__main__":
    df = pd.read_csv("dataset/final_url_dataset.csv")
    features = [extract_lexical_features_v5(url) for url in tqdm(df["url"], desc="🔍 V5 특징 추출 중")]
    feature_df = pd.DataFrame(features, columns=get_feature_names_v5())
    df_with_features = pd.concat([df.reset_index(drop=True), feature_df], axis=1)
    df_with_features.to_csv("dataset/final_url_dataset_with_features_v5.csv", index=False)
    print("✅ 저장 완료: dataset/final_url_dataset_with_features_v5.csv")
